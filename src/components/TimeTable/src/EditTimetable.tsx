...
const EditTimetable = ({data, isRendered, onChangeStartTime, onChangeEndTime}: Props) => {
  const [list, setList] = React.useState<Schedule[]>(data)
  ...
  React.useEffect(() => {
    if (rendered) {
      const updatedList = schedule.schedule_id ? data.filter(item => item.schedule_id !== schedule.schedule_id) : data

      setList(updatedList)
    }
  }, [rendered, data, schedule.schedule_id])
  ...
  return (
    <View style={containerStyle}>
      <View style={wrapperStyle}>
        ...
        <Pressable style={styles.editContainer} onPress={clickBackground} onLayout={() => setRendered(true)}>
          <EditSchedulePie
            data={schedule}
            scheduleList={data}
